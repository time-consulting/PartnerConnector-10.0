...
export default function MobileShareSheet({ 
  ...
  const shareTemplates: ShareTemplate[] = [
    {
      ...
      platform: 'linkedin',
      message: `Hi! I've been growing my business with this amazing partner program. As a ${userStats.level}, I've earned £${userStats.earnings} so far. Would you like to join my team? ${dealUrl}`,
      personalizable: true,
      successRate: 78,
    ...
    },
    {
      ...
      platform: 'whatsapp',
      message: `Hey! 👋 Found this cool way to earn extra income. I'm already at ${userStats.level} level and loving it! Check it out: ${dealUrl}`,
      personalizable: true,
      successRate: 85,
      category: 'messaging'
    },
    ...
    },
    {
      ...
      platform: 'twitter',
      message: `🚀 Just reached ${userStats.level} status! Building an amazing team and earning great commissions. Join me on this journey! #PartnerProgram #BusinessOpportunity ${dealUrl}`,
  ...
  const quickActions = [
    {
      ...
      icon: Copy,
      color: 'bg-gray-500',
      description: 'Copy deal link to clipboard'
    },
    {
      id: 'qr-code',
      name: 'QR Code',
  ...
  const handleQuickShare = async (optionId: string, template?: ShareTemplate) => {
    const message = template?.message || customMessage || `Join me in this amazing partner program! ${dealUrl}`;
    ...
    try {
      switch (optionId) {
        ...
        case 'email':
          ...
          break;
        case 'linkedin':
          window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(dealUrl)}`);
          break;
        case 'twitter':
          window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(message)}`);
          break;
        case 'telegram':
          window.open(`https://t.me/share/url?url=${encodeURIComponent(dealUrl)}&text=${encodeURIComponent(message)}`);
          break;
        case 'facebook':
          window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(dealUrl)}`);
          break;
        case 'copy-link':
      ...
      toast({
        ...
        description: `Your deal link has been shared via ${shareOptions.find(o => o.id === optionId)?.name}`,
      });
    } catch (error) {
      toast({
        title: "Share Failed",
