...
export default function MobileShareSheet({ 
  ...
  const shareTemplates: ShareTemplate[] = [
    {
      ...
      platform: 'linkedin',
      message: `Hi! I've been growing my business with this amazing partner program. As a ${userStats.level}, I've earned £${userStats.earnings} so far. Would you like to join my team? ${dealsUrl}`,
      personalizable: true,
      successRate: 78,
    ...
    },
    {
      ...
      platform: 'whatsapp',
      message: `Hey! 👋 Found this cool way to earn extra income. I'm already at ${userStats.level} level and loving it! Check it out: ${dealsUrl}`,
      personalizable: true,
      successRate: 85,
      category: 'messaging'
    },
    ...
    },
    {
      ...
      platform: 'twitter',
      message: `🚀 Just reached ${userStats.level} status! Building an amazing team and earning great commissions. Join me on this journey! #PartnerProgram #BusinessOpportunity ${dealsUrl}`,
  ...
  const handleQuickShare = async (optionId: string, template?: ShareTemplate) => {
    const message = template?.message || customMessage || `Join me in this amazing partner program! ${dealsUrl}`;
    ...
    try {
      switch (optionId) {
        ...
        case 'email':
          ...
          break;
        case 'linkedin':
          window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(dealsUrl)}`);
          break;
        case 'twitter':
          window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(message)}`);
          break;
        case 'telegram':
          window.open(`https://t.me/share/url?url=${encodeURIComponent(dealsUrl)}&text=${encodeURIComponent(message)}`);
          break;
        case 'facebook':
          window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(dealsUrl)}`);
          break;
        case 'copy-link':
          navigator.clipboard.writeText(dealsUrl);
          toast({
            title: "Link Copied!",
